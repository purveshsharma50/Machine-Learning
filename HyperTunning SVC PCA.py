...
# and to hypertunning and optimizing  the parameters using PCA
# #*************************************************************************************
...
# We need # We need all features and class
...
a=('Test Accuracy: %.3f' % pipe_svc.score(X_test, y_test))
...
for k, (train, test) in enumerate(kfold):
    ...
    scores.append(score)
    b=('Fold: %2d, Class dist.: %s, Acc: %.3f' % (k+1, 
                                                 np.bincount(y_train[train]), score))
c=('\nCV accuracy: %.3f +/- %.3f' % (np.mean(scores), np.std(scores)))

scores = cross_val_score(estimator=pipe_svc,X=X_train,y=y_train, cv=20,n_jobs=-1)
d=('CV accuracy scores: %s' % scores)

e=('CV accuracy: %.3f +/- %.3f' % (np.mean(scores), np.std(scores)))
...
with open("SVC hypertunning PCA_output.txt", "w") as text_file:
