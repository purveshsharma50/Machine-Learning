...
#
# Objective:
# To Classify and predict Cars dataset using DecisionTree for high safety >= HighPrice
# #*************************************************************************************
...
#training and prediction using DecisionTree

tree = DecisionTreeClassifier(max_depth=4, random_state=1) 
tree.fit(X_train_norm, y_train)
X_New = np.vstack((X_train_std, X_test_norm)) 
y_New = np.hstack((y_train, y_test)) 


y_New = tree.predict(X_test_norm)  #predicting test results
...
print('Misclassified samples: %d' % (y_test != y_New).sum())

print('Accuracy For Tree: %.2f' % accuracy_score(y_test, y_New))
print('Test Accuracy for Tree: %.2f' % tree.score(X_test_norm, y_test))
print('Train Accuracy for Tree: %.2f' % tree.score(X_train_norm, y_train))       
# plot data
...
X_combined_std = np.vstack((X_train_norm, X_test_norm))
